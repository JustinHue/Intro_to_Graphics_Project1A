"""
    ...
    Revision 0.3.0
        ...
        -> Added functionality which loads level.dat on start up. You can also load using 
           ctrl + l
        -> Added loading message when ctrl + l is pressed
        -> Added feature to turn grids on and off. 
        -> Added reset method to tile map
        -> Adding level maker configuration file with entity config ([enemy-name],[key],[img-path])
        -> Tile map loads entity configuration on start up.
        -> Implemented full functionality to add entities to the map
        -> Added functionality to remove entities on the map (right click entity)
        -> Can now add shyguys and goombas to the map
        -> Added entities to map file when saving
        -> Entities now load when loading level data file
        -> Fixed issue where loading a bigger map than what is currently loaded will cause vertical expansion 
           instead o horizontal expansion and vice versa 
        -> Added coin entity which can be added using alt + c
           ...
           mousePos[1] > 0 and mousePos[1] < self.rect.bottom:        
            ...
            if event.type == pygame.KEYDOWN:
                ...
                #These are assocciated with entity creations
                    ...
                    for info in self.entityInfo:
                        if info[1] == "g" and keys[pygame.K_g]:
                            self.entities.append([info[0], self.scrollx + mousePos[0] - self.rect.left, 
                                                  self.scrolly + mousePos[1] - self.rect.top])
                            self.__redraw()
                        if info[1] == "s" and keys[pygame.K_s]:
                            self.entities.append([info[0], self.scrollx + mousePos[0] - self.rect.left, 
                                                  self.scrolly + mousePos[1] - self.rect.top])
                            self.__redraw()
                        if info[1] == "c" and keys[pygame.K_c]:
                            self.entities.append([info[0], self.scrollx + mousePos[0] - self.rect.left, 
                                                  self.scrolly + mousePos[1] - self.rect.top])
                            self.__redraw()
